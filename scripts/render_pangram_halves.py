...
def reader_visible_with_breaks(markdown: str) -> str:
    ...
    visible = "\n".join(out).strip()
    if re.sub(r"\s+", " ", visible).strip() != rrv.reader_visible_text(markdown):
...
def split_near_half(visible: str, headings: set[str]) -> tuple[str, str, str | None]:
    blocks = visible.split("\n\n")
    if len(blocks) < 2:
        raise ValueError("not enough visible blocks to split")
    words = [len(block.split()) for block in blocks]
    target = sum(words) / 2
    cumulative = 0
    choices: list[tuple[float, int]] = []
    for i in range(1, len(blocks)):
        cumulative += words[i - 1]
        choices.append((abs(cumulative - target), i))
    _, split_at = min(choices)
...
def main() -> int:
    ...
    part1, part2, split_heading = split_near_half(visible, _article_headings(markdown))
    canonical = rrv.reader_visible_text(markdown)
    if re.sub(r"\s+", " ", part1 + " " + part2).strip() != canonical:
        raise ValueError("split halves do not reconstruct the reader-visible authority")

    args.part1.write_text(part1, encoding="utf-8")
    args.part2.write_text(part2, encoding="utf-8")
    manifest = {
        "schema_version": 1,
        ...
        "part2_first_line": part2.splitlines()[0],
