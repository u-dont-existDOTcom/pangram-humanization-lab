...
The corrected target retry later published all queued records plus the new status snapshot in one attempt. Remote commit `3ca6ad75bafebb1cb3940b82e00d5b5982880e32` contains three dated allowlisted records, `LATEST.json` points to corrected-program snapshot `bb11caa0551587e3c56c04e6aec8c8f4f575dc24851e5ac89080161f3cd89f7c`, and the local result reported `queued=0`. All remote records passed exact-key, run-ID, privacy-category, local-path, and credential-token checks.

The next automatic `runtime-resume` record, `65e71d7603a2c664c389753d88ee32e646d8d18fbe038e9bbf9ef82aee16a679`, proved transport remained healthy and exposed a separate same-thread recovery defect: it returned the unchanged three-move `GENERATION_DEAD_END` with retry count four, no provider activity, and an empty failure origin. The preserved SQLite history showed that the legacy terminal predates origin normalization. Recovery now infers the replay node from the newest current-lineage `machine_failure` checkpoint rather than requiring the terminal field or replaying an older lineage.
