...
The browser interaction itself is local: headed Brave/Chromium via Playwright -> pangram.com. GitHub can be used for code/versioning and durable receipts, but it is not the browser transport. **When the current Chat surface has direct authorized connected-computer / Remote Desktop Commander access, this local GUI workflow stays in Chat and does not require a Work/Codex handoff merely because it uses a terminal, browser, headed GUI, or another machine.** A worker must not infer `GUI unavailable` merely because a particular Chat context lacks the local command/computer-use bridge.
