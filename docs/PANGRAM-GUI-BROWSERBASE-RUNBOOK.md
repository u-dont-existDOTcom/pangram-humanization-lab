...
Authenticated cross-session Pangram persistence is live-verified: the corrected bootstrap completed and the read-only `verify` command reached the authenticated detector in a fresh session, then closed normally without submitting text. One authorized 121-word smoke submission completed in Pangram; after the original runner was interrupted with exit code `130`, no-submit recovery captured the existing History report successfully. The content-bound result records Pangram 4.0, 121 words, 100% AI, the current short-text Overview layout, and a valid Playwright print-to-PDF fallback. The runner now recognizes that Overview's `of this text is AI` completion marker and parses the layout directly. Do not submit that hash again. A native Pangram download and long-document report layout remain unverified.

The short-text live smoke gate is complete. Do not infer native-download or long-document behavior from the print-fallback short-text result.
