...
**Reader-visible representation gate:** detector certification must use the **reader-visible** text surface Pangram will actually evaluate. For Markdown article work, **raw Markdown is diagnostic only**: strip Markdown syntax, link destinations, and other non-visible source markup before the certification call, and test the resulting **visible plaintext**. For Substack, certify the rendered/reader-visible text, including any embed or card text that Pangram actually surfaces. A raw-source pass never certifies a different rendered publication boundary.
