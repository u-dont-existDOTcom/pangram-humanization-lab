...
5. `state/WORKING-LESSONS-SUPPLEMENT-2026-08-17-IDIOLECT.md` — IER-derived idiolect-preservation lessons: minimum rewrite dose, natural-owner corpus provenance, topic controls, terminology discipline, and the rule that detector green does not establish preserved byline voice.
6. `state/ROMANCE-OWNER-STYLE-BAN-FRAGMENT-REVERSAL-2026-08-15.md` — exact source/scope for the direct Joel hard ban on generated `X. Not really/not quite/not exactly Y.` cadence; owner preference, not Pangram evidence.
7. `state/ROMANCE-OPENING-PERSONAL-PROVENANCE-2026-08-15.md` — current owner-final Romance opening plus owner-reported HIGH→MEDIUM detector controls when explicit personal/source provenance is removed; treat as a rhetorical-function/provenance hypothesis, not a `my` lexical rule.
8. `docs/CHATGPT-OPERATING-GUIDE.md` — current execution/completion contract, including the six-paid-call per-section cap and durable lesson-review fallback.
9. For exact evidence behind a lesson, inspect the cited incident note and experiment/result JSON rather than generalizing from the summary.
...
- **Idiolect preservation is separate from detector passing:** named-byline humanization should preserve distinguishable author signal in addition to meaning and requested detector performance. Prefer minimum rewrite dose, keep natural-owner reference corpora uncontaminated, use style-sensitive/topic-controlled attribution when calibrated, and never treat a `preserve voice` prompt or Pangram green result as proof that Joel's voice survived. See `state/WORKING-LESSONS-SUPPLEMENT-2026-08-17-IDIOLECT.md` and `docs/IDIOLECT-PRESERVATION-PROTOCOL.md`.
