...
4. `state/ROMANCE-OWNER-STYLE-BAN-FRAGMENT-REVERSAL-2026-08-15.md` — direct Joel hard ban for generated byline prose on the formulaic `X. Not really/not quite/not exactly Y.` clipped reversal cadence; owner preference, not Pangram evidence.
5. `docs/CHATGPT-OPERATING-GUIDE.md` — current execution/completion contract, including the six-paid-call per-section cap and durable lesson-review fallback.
6. For exact evidence behind a lesson, inspect the cited incident note and experiment/result JSON rather than generalizing from the summary.
