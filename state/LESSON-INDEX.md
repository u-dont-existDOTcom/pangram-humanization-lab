...
- **SUPERSEDED — Human-surplus hard gate:** v1's 14/20 result correctly showed that model-simulated Human-looking devices are not automatic positive evidence, but its first correction overreached. Untouched holdout v2 scored 10/20 overall—1/10 Human, 9/10 AI—after content-neutralized scaffold analysis became a hard veto. Do not infer AI merely from a clean functional skeleton or require Human prose to break that skeleton with inefficiency/surplus. The abstract-rule approach has reached method escalation; move to literal contrastive Human/AI calibration. A materially changed critic still requires a new untouched 20/20 provenance holdout before gating Pangram. See `state/generation/critic-benchmark-20260922/V1-RESULT-AND-RUBRIC-DIAGNOSIS.md` and `state/generation/critic-holdout-v2-20260922/RESULT-AND-METHOD-DIAGNOSIS.md`.
