...
obj=json.loads((ROOT/"antecedent-v2-human-control-2.json").read_text())
c=obj["case"]
...
print(json.dumps({"event":"antecedent_human2_start","case_id":c["case_id"],"model":model}),flush=True)
...
print(json.dumps({"event":"antecedent_human2_result","case_id":c["case_id"],"model":raw.get("model"),"response_id":raw.get("id"),"content":content,"parsed":parsed,"parse_error":parse_error,"usage":raw.get("usage")},ensure_ascii=False),flush=True)
print(json.dumps({"event":"antecedent_human2_end"}),flush=True)
